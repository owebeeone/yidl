...
for field in ctx.records(FieldsCollection):
    ...
    ctx.write(
        ...
        policy=RejectDuplicate,
    )""", file_name='tests/data/yidl/yidl_transactional_lifecycle/lifecycle_transient.yidl', line_number=32, keep_names=('ctx', 'FieldsCollection', 'TxGroupsCollection', 'IndexedTransientFieldsCollection', 'IndexedTransientField', 'RejectDuplicate')), 'TransientCurrentProperty': astichi_template(from_astichi_code("""\
@property
def property_getter_name__astichi_arg__(self):
    return self._y_state.astichi_ref(external=current_slot)""", file_name='tests/data/yidl/yidl_transactional_lifecycle/lifecycle_transient.yidl', line_number=75)), 'TransientWorkingStateAssignment': astichi_template(from_astichi_code('astichi_pass(state, outer_bind=True).astichi_ref(external=working_slot)._ = VOID', file_name='tests/data/yidl/yidl_transactional_lifecycle/lifecycle_transient.yidl', line_number=81, keep_names=('VOID',))), 'TransientFacadeProperty': astichi_template(from_astichi_code("""\
@property
def property_getter_name__astichi_arg__(self):
    state = self._y_state
    if state.astichi_ref(external=working_slot) is not VOID:
...
def property_setter_name__astichi_arg__(self, value):
    state = self._y_state
    state._y_ensure_working_transaction(astichi_bind_external(tx_index))
    state.astichi_ref(external=working_slot)._ = value""", file_name='tests/data/yidl/yidl_transactional_lifecycle/lifecycle_transient.yidl', line_number=87, keep_names=('VOID',))), 'TransientWorkingDefaultFactoryProperty': astichi_template(from_astichi_code("""\
@property
def property_getter_name__astichi_arg__(self):
    state = self._y_state
    if state.astichi_ref(external=working_slot) is not VOID:
