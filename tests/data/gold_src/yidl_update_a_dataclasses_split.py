...
def validate_case(sources: Mapping[str, str]) -> None:
    ...
    same = widget_class(3, scale=99, tags=["different"])
    different = widget_class(4, scale=5)
    overridden = widget_class(3, level=8, scale=5)

    assert first.count == 3
    assert first.level == 7
    assert first.tags == []
    assert first.hidden == "secret"
    assert not hasattr(first, "scale")
    ...
    assert widget_class.__match_args__ == ("count", "level", "tags", "scale")
    ...
    assert widget_class.__dataclass_fields__["scale"]["kind"] == "initvar"
    assert widget_class.__dataclass_fields__["kind"]["kind"] == "classvar"
    assert "kind" in widget_class.__annotations__
    assert "kind" not in widget_class.__match_args__
    assert "scale" not in repr(first)
    assert "kind" not in repr(first)
    assert "setattr__(self, 'scale'" not in source
...
def _container(namespace: Mapping[str, object]) -> object:
    ...
    classvar = namespace["ClassVarField"]
...
if __name__ == "__main__":
    raise SystemExit(
        run_multi_source_case(
            "yidl_update_a_dataclasses_split.py",
            render_case,
            validate_case,
        )
    )
