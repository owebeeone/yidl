...
store with stable query and write primitives. It is not the semantic
interpreter for every DDS operation.
...
functions, freezes the result, and then feeds the resolved data to the Astichi
build mapper.
...
- queries the frozen data to select Astichi generation resources
...
- final fact indexes
- read-only query APIs consumed by the Astichi build mapper
...
Every stored record contributes equality facts for its collection:
...
The builder indexes facts as records are inserted. Generated operation functions
then use Eq-only lookup:

```python
builder.matching(Fields, Init.eq(True))
```

Required builder and resolved-container accessors:

- `records(collection) -> tuple[object, ...]`
- `single(collection) -> object | None`
- `by_identity(collection, value) -> object | None`
- `matching(collection, *conditions) -> tuple[object, ...]`

`matching(...)` must remain Eq-only. Richer concepts should be materialized into
derived properties or derived records first.
...
Filters are generated operation code over container facts, not DDS mutation.

The DDS defines the filter condition during specification. The generated module
emits code that calls:
...
coordinates operation functions. The container should stay as the stable fact
